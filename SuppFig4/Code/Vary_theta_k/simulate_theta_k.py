...
path_data = 'Data_simulate_theta_k/'
path_plot = 'Plots_simulate_theta_k/'
...
k_values = np.linspace(1-1e-5, 1.6, 401)
...
if doCompute:
	...
	asymmetry = np.zeros(( N, N, len(theta_values), len(k_values) ))
	lag = np.zeros(( N, N, len(theta_values), len(k_values) ))

	variance = np.zeros(( N, len(theta_values), len(k_values) ))
	tau = np.zeros(( N, len(theta_values), len(k_values) ))

	nan = np.zeros(( N, len(theta_values), len(k_values) ))
	...
	for ii_theta, theta in enumerate(theta_values):
		...
		for ii_k, k in enumerate(k_values):
			...
			asymmetry[:,:,ii_theta,ii_k] = th.AsymmetryScore(cross.real, tau_values)
			lag[:,:,ii_theta,ii_k] = th.PrincipalLag(cross.real, tau_values)
			variance[:,ii_theta,ii_k] = np.max(cross[np.arange(N), np.arange(N),:], 1)
			tau[:,ii_theta,ii_k] = (int(Ntau/2) - var.WidthHalfHeight(cross_input[:,0]))*(2*tau_m/Ntau)
			...
			if (len(np.where(cross_input[0,0,:]<0)[0])==0 and len(np.where(cross_input[2,0,:]<0)[0])==0): 
				...
				nan[:,ii_theta,ii_k] = nan_value

	# Compute boundary

	boundary = np.zeros(len(k_values))

	for ii_k, h in enumerate(k_values):
		if len(np.where(np.isnan(nan[0,:,ii_k])==True)[0])>0:
			boundary[ii_k] = theta_values[np.min(np.where(np.isnan(nan[0,:,ii_k])==True)[0])]
		else:
			boundary[ii_k] = 10
...
plt.imshow(variance[0,:,:].T, origin='lower', \
	extent = (np.min(theta_values), np.max(theta_values), np.min(k_values), np.max(k_values)), aspect='auto', \
	interpolation = 'nearest', vmin = 0, vmax = np.max(np.fabs(variance[0,:,:])), cmap=cmap)

plt.axvline(x=45, ls='--', color='0', linewidth=0.5)

plt.plot(boundary, k_values, color='0', linewidth=0.7)
...
plt.ylim(np.min(k_values), np.max(k_values))
plt.yticks([np.min(k_values), np.max(k_values)])

plt.xlabel(r'Input angle')
plt.ylabel(r'Inh dominance')
