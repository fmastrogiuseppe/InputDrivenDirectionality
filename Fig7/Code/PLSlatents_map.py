...
if __name__ == "__main__":
	...
	# Apply PLS and compute corresponding cross-covariances (each stimulus separately, in parallel)
	...
	if doCompute:
		...
		for ii_session in range(Nsessions):
			for ii_stimulus in range(Nstimuli):
				...
				semaphore.acquire()
				process = multiprocessing.Process(target=correl.CCAMap, args=(ii_session, ii_stimulus, Nstimuli, stimulus_pairs, \
					time_step, time_step_forward, delay_step, max_delay, timebin, 'cov', path_data, semaphore))
				process.start()
				p.append(process)
