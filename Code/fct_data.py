...
threshold_rate = 0.5 # Used to exclude silent neurons (in Hz)
threshold_FF = 1e10
...
def Bin(spikes, bin_size):
	...
	spikes_binned = np.mean(np.reshape(spikes, \
					(spikes.shape[0], spikes.shape[1], int(spikes.shape[2]/float(bin_size)), bin_size)), 3)
...
def ComputeTuningScore(data, input_stimulus, Nstimuli, Nrepet):

	# Compute tuning curves

	tuning = np.zeros(( data.shape[0], Nstimuli))
			
	for ii_stimulus in np.arange(Nstimuli):
		tuning[:,ii_stimulus] = np.mean(data[:,ii_stimulus*Nrepet:(ii_stimulus+1)*Nrepet,:], (1,2))

	# Compute score

	score = (tuning[:,input_stimulus]-np.min(tuning, 1))/(np.max(tuning, 1) - np.min(tuning, 1))
...
def AUC(cross, tau_values):
	...
	for ii_neuron_1 in range(N):
		for ii_neuron_2 in range(N):
			auc[ii_neuron_1, ii_neuron_2] = (tau_values[1]-tau_values[0]) \
											* (np.sum(cross[ii_neuron_1, ii_neuron_2,np.where(tau_values>0)[0]]) + np.sum(cross[ii_neuron_1, ii_neuron_2,np.where(tau_values<0)[0]]))

	return auc

#

def Amplitude(cross, tau_values):

	return np.nanmax(cross, 2) 
