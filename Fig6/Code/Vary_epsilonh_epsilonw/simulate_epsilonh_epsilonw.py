...
path_data = 'Data_simulate_epsilonh_epsilonw/'
path_plot = 'Plots_simulate_epsilonh_epsilonw/'
...
epsilon_w_values = np.linspace(1e-2, 1-1e-3, 401)
...
if doCompute:
	...
	asymmetry = np.zeros(( N, N, len(epsilon_h_values), len(epsilon_w_values) ))
	lag = np.zeros(( N, N, len(epsilon_h_values), len(epsilon_w_values) ))

	asymmetry_latent = np.zeros(( Nlatents, Nlatents, len(epsilon_h_values), len(epsilon_w_values) ))
	lag_latent = np.zeros(( Nlatents, Nlatents, len(epsilon_h_values), len(epsilon_w_values) ))

	variance_ratio = np.zeros(( 2, len(epsilon_h_values), len(epsilon_w_values) ))
	crosscovariance_ratio = np.zeros(( len(epsilon_h_values), len(epsilon_w_values) ))
	tau_ratio = np.zeros(( len(epsilon_h_values), len(epsilon_w_values) ))
	...
	for ii_epsilon_h, epsilon_h in enumerate(epsilon_h_values):
		...
		for ii_epsilon_w, epsilon_w in enumerate(epsilon_w_values):
			...
			if (len(np.where(cross_input[0,0,:]<0)[0])==0 and len(np.where(cross_input[2,0,:]<0)[0])==0) \
				...
				asymmetry[:,:,ii_epsilon_h,ii_epsilon_w] = th.AsymmetryScore(cross.real, tau_values)
				lag[:,:,ii_epsilon_h,ii_epsilon_w] = th.PrincipalLag(cross.real, tau_values)

				asymmetry_latent[:,:,ii_epsilon_h,ii_epsilon_w] = th.AsymmetryScore(cross_latent.real, tau_values)
				lag_latent[:,:,ii_epsilon_h,ii_epsilon_w] = th.PrincipalLag(cross_latent.real, tau_values)

				variance_ratio[0,ii_epsilon_h,ii_epsilon_w] = np.max(cross_latent[1,1,:])/np.max(cross_latent[0,0,:])
				variance_ratio[1,ii_epsilon_h,ii_epsilon_w] = np.max(cross_latent[3,3,:])/np.max(cross_latent[2,2,:])
				crosscovariance_ratio[ii_epsilon_h,ii_epsilon_w] = np.max(cross_latent[1,3,:])/np.max(cross_latent[0,2,:])

			else:

				asymmetry[:,:,ii_epsilon_h,ii_epsilon_w] = nan_value
				lag[:,:,ii_epsilon_h,ii_epsilon_w] = nan_value

				asymmetry_latent[:,:,ii_epsilon_h,ii_epsilon_w] = nan_value
				lag_latent[:,:,ii_epsilon_h,ii_epsilon_w] = nan_value

				variance_ratio[:,ii_epsilon_h,ii_epsilon_w] = np.nan
				crosscovariance_ratio[ii_epsilon_h,ii_epsilon_w] = np.nan


	# Compute boundary

	boundary = np.zeros(len(epsilon_w_values))

	for ii_epsilon_w, epsilon_w in enumerate(epsilon_w_values):
		if len(np.where(np.isnan(crosscovariance_ratio[:,ii_epsilon_w])==True)[0])>0:
			boundary[ii_epsilon_w] = epsilon_h_values[np.min(np.where(np.isnan(crosscovariance_ratio[:,ii_epsilon_w])==True)[0])]
		else:
			boundary[ii_epsilon_w] = 10
...
plt.imshow(asymmetry_latent[0,2,:,:].T, origin='lower', \
	extent = (np.min(epsilon_h_values), np.max(epsilon_h_values), np.min(epsilon_w_values), np.max(epsilon_w_values)), aspect='auto', \
...
plt.plot(boundary, epsilon_w_values, color='0', linewidth=0.7)
